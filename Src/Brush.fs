...
module Brush =
    ...
    /// result will be clamped between 0 and 255
    let brighter (amount:int) (br:SolidColorBrush)  = SolidColorBrush(Color = changeLuminance amount br.Color)

    /// Removes bytes from each color channel to increase darkness,
    /// result will be clamped between 0 and 255
    let darker  (amount:int) (br:SolidColorBrush)  = SolidColorBrush(Color = changeLuminance -amount br.Color)


/// Utility functions for System.Windows.Media.Pen
module Pen =
    open System
    open System.Windows.Media // for Pen
    open Brush

    /// Make it thread-safe and faster
    let inline freeze(pen:Pen)=
        if not pen.IsFrozen then
            if pen.CanFreeze then pen.Freeze()
            //else                 eprintfn "Could not freeze Pen: %A" pen
        pen
